...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < rMatrix.length; i += 1) {
    for (let j = i + 1; j < rMatrix.length; j += 1) {
      const temp = rMatrix[i][j];
      rMatrix[i][j] = rMatrix[j][i];
      rMatrix[j][i] = temp;
    }
  }
  for (let i = 0; i < rMatrix.length; i += 1) {
    for (let j = 0; j < rMatrix.length / 2; j += 1) {
      const temp = rMatrix[i][j];
      rMatrix[i][j] = rMatrix[i][rMatrix.length - 1 - j];
      rMatrix[i][rMatrix.length - 1 - j] = temp;
    }
  }
  return rMatrix;
}
